...
def is_valid_move(maze, row, col):
    # Verificar si la celda está dentro de los límites del laberinto y si es un pasillo
...
def bfs(maze, start, end, canvas):
    ...
    return -1, None  # No se encontró ruta hasta la celda de salida
...
def find_shortest_path(maze, start, end, canvas):
    ...
    if distance == -1:
        return None, -1  # No se encontró una ruta válida
    else:
...
root.geometry('{}x{}'.format((cols+1)*cell_size, (rows+2)*cell_size))  # Ajustar tamaño de ventana
